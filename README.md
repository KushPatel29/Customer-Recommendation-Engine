...
| Recommender | Hit-rate@10 |
|---|---|
| **Collaborative filtering (this engine)** | **84.9%** |
| Popularity baseline ("suggest the bestsellers") | 75.4% |

CF wins by 1.13x — and the honest footnote is that popularity is a strong
baseline on a 38-SKU catalog, which is exactly why you measure instead of
assuming. The test suite enforces `CF > popularity` as a hard invariant:
if a code change breaks the model's edge, CI fails.
...
engine/           recommend.py — CF cross-sell, growth targets, basket affinity
evaluation/       holdout protocol: hide 25% of SKUs, measure hit-rate@10 vs baseline
tests/            7 invariants: no already-owned recs, symmetric similarity,
